...
export function CorralCard({
  ...
  async function handleEliminarCorral() {
    setBorrando(true);
    try {
      await eliminarCorral(corral.id);
      onCambio();
    } finally {
      setBorrando(false);
    ...
    <>
      <Card className="overflow-hidden">
        ...
        {expandido && (
          <div className="px-5 pb-5 space-y-4 border-t border-earth-700/10 pt-4" onClick={(e) => e.stopPropagation()}>
            ...
            {esAdministrador && (
              <div className="pt-2 border-t border-earth-700/10">
      ...
      {confirmandoBorrado && (
        <div className="fixed inset-0 z-[60] flex items-end md:items-center justify-center">
          <div className="absolute inset-0 bg-earth-950/50 backdrop-blur-[2px]" onClick={() => setConfirmandoBorrado(false)} />
          <div className="relative bg-white w-full md:w-[420px] rounded-t-card md:rounded-card p-6 shadow-card-lg">
            <p className="font-display font-semibold text-lg text-earth-950 mb-2">
              ¿Eliminar "{corral.nombre}"?
            </p>
            <p className="text-sm text-earth-700/60 mb-5">
              Esta acción no se puede deshacer. El historial de entregas de este corral quedará asociado a un corral eliminado.
            </p>
            <div className="flex gap-2">
              <Button variant="ghost" className="flex-1" onClick={() => setConfirmandoBorrado(false)}>
                Cancelar
              </Button>
              <Button variant="danger" className="flex-1" onClick={handleEliminarCorral} disabled={borrando}>
