  ...
  return (
    <div className="max-w-5xl mx-auto px-4 md:px-8 py-6 md:py-10 pb-40">
      <header className="flex items-center justify-between mb-6">
        ...
        {esAdministrador && <Button onClick={() => setModalAbierto(true)}>+ Nuevo corral</Button>}
      </header>
      ...
      {!loading && !error && corrales.length === 0 && (
        <EstadoVacio onCrear={() => setModalAbierto(true)} puedeCrear={esAdministrador} />
      )}
...
function EstadoVacio({ onCrear, puedeCrear }: { onCrear: () => void; puedeCrear: boolean }) {
  return (
    <Card className="py-14 text-center">
      <CardContent>
        ...
        {puedeCrear ? (
          ...
          <p className="text-earth-700/60 text-sm">Esperá a que un Administrador cree los corrales.</p>
        )}
      </CardContent>
    </Card>
  );
